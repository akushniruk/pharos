...
export function useChartData(agentIdFilter?: string) {
  ...
  const processEventBuffer = () => {
    ...
    eventsToProcess.forEach(event => {
      ...
      if (bucket) {
        ...
        const agentKey = createAgentId(event.source_app, event.session_id, event.agent_id);
        bucket.sessions[agentKey] = (bucket.sessions[agentKey] || 0) + 1;
      } else {
  ...
  const reaggregateData = () => {
    ...
    relevantEvents.forEach(event => {
      ...
      if (bucket) {
        ...
        const agentKey2 = createAgentId(event.source_app, event.session_id, event.agent_id);
        bucket.sessions[agentKey2] = (bucket.sessions[agentKey2] || 0) + 1;
      } else {
        const toolEvents: Record<string, number> = {};
        if (event.payload?.tool_name) {
          toolEvents[`${event.hook_event_type}:${event.payload.tool_name}`] = 1;
  ...
  // Helper to create unique agent ID from source_app + session_id (or agent_id for subagents)
  const createAgentId = (sourceApp: string, sessionId: string, agentId?: string): string => {
    if (agentId) {
      return `${sourceApp}:${agentId.slice(0, 8)}`;
    }
  ...
  const uniqueAgentIdsInWindow = computed(() => {
    ...
    allEvents.value.forEach(event => {
      if (event.timestamp && event.timestamp >= cutoffTime) {
        const agentId = createAgentId(event.source_app, event.session_id, event.agent_id);
